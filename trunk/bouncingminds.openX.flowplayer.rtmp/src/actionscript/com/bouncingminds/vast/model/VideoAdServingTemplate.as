...
package com.bouncingminds.vast.model {
	 ...
	 * @author Paul Schulz
	 */
	public class VideoAdServingTemplate extends DebugObject {
		protected var _xmlLoader:URLLoader = null;
		protected var _listener:VASTLoadListener = null;
		 ...
		 */		
		public function VideoAdServingTemplate(listener:VASTLoadListener=null, request:OpenXVASTAdRequest=null) {
		...
		public function load(request:OpenXVASTAdRequest):void {
			doLog("Loading VAST data from Open X server via " + request.formRequest(), DebugObject.DEBUG_VAST_TEMPLATE);
		...
		protected function templateLoaded(e:Event):void {
			doLog("Loaded " + _xmlLoader.bytesLoaded + " bytes for the VAST template", DebugObject.DEBUG_VAST_TEMPLATE);
			doTrace(_xmlLoader, DebugObject.DEBUG_VAST_TEMPLATE);
			_templateData = _xmlLoader.data;
			parseFromRawData(_templateData);
			doLogAndTrace("VAST Template parsed and ready to use", this, DebugObject.DEBUG_VAST_TEMPLATE);
			_dataLoaded = true;
			if(_listener != null) _listener.onTemplateLoaded(_templateData);
		}
		
		protected function errorHandler(e:Event):void {
			doLog("HTTP ERROR: " + e.toString(), DebugObject.DEBUG_VAST_TEMPLATE);
			_loadFailed = true;
		...
		/**
		 * Identifies whether or not an error occurred when trying to load the data from the ad server
		 * 
		 * @param loadedStatus a boolean value that identifies whether or not an error occurred 
		 */
		...
		public function parseFromRawData(rawData:*):void {
 			...
 			doLog("Number of video ad serving templates returned = " + xmlData.length(), DebugObject.DEBUG_VAST_TEMPLATE);
 			if(xmlData.length() > 0) {
		...
		private function parseAdSpecs(ads:XMLList):void {
			doLog("Parsing " + ads.length() + " ads in the template...", DebugObject.DEBUG_VAST_TEMPLATE);
			for(var i:int=0; i < ads.length(); i++) {
				...
				else doLog("No InLine tag found for Ad - " + adIds[0] + " - ignoring this entry", DebugObject.DEBUG_VAST_TEMPLATE);	
			}
			doLog("Parsing DONE", DebugObject.DEBUG_VAST_TEMPLATE);
		}
		
		private function parseAdSpecification(adRecordPosition:int, adId:String, ad:XML):VideoAd {
			doLog("Parsing Ad record at position " + adRecordPosition + " with ID " + adId, DebugObject.DEBUG_VAST_TEMPLATE);
			doLog("Ad has " + ad.children().length() + " attributes defined - see trace", DebugObject.DEBUG_VAST_TEMPLATE);
			doTrace(ad, DebugObject.DEBUG_VAST_TEMPLATE);
			if(ad.children().length() > 0) {
				...
				vad.error = ad.Error;
				var i:int;
				doLog("Parsing impression data...", DebugObject.DEBUG_VAST_TEMPLATE);
				...
				doLog("Parsing TrackingEvent data...", DebugObject.DEBUG_VAST_TEMPLATE);
				...
				if(ad.Video != undefined) {
					doLog("Parsing Video data...", DebugObject.DEBUG_VAST_TEMPLATE);
					...
					if(ad.Video.VideoClicks != undefined) {
						...
						if(ad.Video.VideoClicks.ClickThrough.children().length() > 0) {
							doLog("Parsing VideoClicks ClickThrough data...", DebugObject.DEBUG_VAST_TEMPLATE);
						...
						if(ad.Video.VideoClicks.ClickTracking.children().length() > 0) {
							doLog("Parsing VideoClicks ClickTracking data...", DebugObject.DEBUG_VAST_TEMPLATE);
						...
						if(ad.Video.VideoClicks.CustomClick.children().length() > 0) {
							doLog("Parsing VideoClicks CustomClick...", DebugObject.DEBUG_VAST_TEMPLATE);
					...
					if(ad.Video.MediaFiles != undefined) {
						doLog("Parsing MediaFiles data...", DebugObject.DEBUG_VAST_TEMPLATE);
				...
				if(ad.NonLinearAds != undefined) {
					doLog("Parsing NonLinearAd data...", DebugObject.DEBUG_VAST_TEMPLATE);
				...
				if(ad.CompanionAds != undefined) {
					doLog("Parsing CompanionAd data...", DebugObject.DEBUG_VAST_TEMPLATE);
				...
				doLog("Parsing ad record " + adId + " done", DebugObject.DEBUG_VAST_TEMPLATE);
				doTrace(vad, DebugObject.DEBUG_VAST_TEMPLATE);
				return vad;
			}
			else doLog("No tags found for Ad " + adId + " - ignoring this entry", DebugObject.DEBUG_VAST_TEMPLATE);
			return null;
		}
		...
		public function getVideoAdWithID(id:String):VideoAd {
			doLog("Looking for a Video Ad " + id, DebugObject.DEBUG_VAST_TEMPLATE);
			if(_ads != null) {
				for(var i:int = 0; i < _ads.length; i++) {
					if(_ads[i].id == id) {
						doLog("Found Video Ad " + id + " - returning", DebugObject.DEBUG_VAST_TEMPLATE);
						return _ads[i];
					}
				}	
				doLogAndTrace("Could not find Video Ad " + id + " in the VAST template", this, DebugObject.DEBUG_VAST_TEMPLATE);
			}
			else doLog("No ads in the list!", DebugObject.DEBUG_VAST_TEMPLATE);
			return null;
		}		
	}
}